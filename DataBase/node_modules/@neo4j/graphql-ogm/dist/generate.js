...
function hasConnectOrCreate(node, schema) {
    for (const relation of node.relationFields) {
        const refNode = utils_1.getReferenceNode(schema, relation);
...
async function generate(options) {
    ...
    options.ogm.neoSchema.nodes.forEach((node) => {
        const pluralized = pluralize_1.default(node.name);
        const camelName = camelcase_1.default(pluralized);
        const upperCamel = graphql_1.upperFirst(camelName);
